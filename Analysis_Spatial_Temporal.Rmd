...
knit: (function(inputFile, encoding) {
  ...
  rmarkdown::render(inputFile,
      encoding=encoding, 
      output_file='Reports/Analysis_Spatial_Temporal.html') })
output:
  html_document:
    toc: true
    toc_float:
...
#We need to compile SAE.R to have the data in the '2_Analysed' folder
#Spatial models
#Load DIC & WAIC of spatial models
load("Data/2_Analysed/Spatial/sp_dic_waic.Rda")
#Load RR of spatial models
load("Data/2_Analysed/Spatial/res_sp.Rda")
#Load hyperparameters of BYM2 spatial model
load("Data/2_Analysed/Spatial/bym2_hp.Rda")

#Spatio-Temporal models
#Load DIC & WAIC of spatio-temporal base models
load("Data/2_Analysed/SpatioTemporal/st_dic_waic_base.Rda")
#Load DIC & WAIC of spatio-temporal models with different type of interactions
load("Data/2_Analysed/SpatioTemporal/st_dic_waic.Rda")
load("Data/2_Analysed/SpatioTemporal/st_dic_waic_sir2.Rda")

#Load the spatio-temporal models with type II interaction (sir & sir2)
load("Data/2_Analysed/SpatioTemporal/ndat_sae_II.Rda")

#Spatial covariates
load("Data/2_Analysed/Covariates/res_sae_raw.Rda")
load("Data/2_Analysed/Covariates/res_sae_urban.Rda")
load("Data/2_Analysed/Covariates/ndat_sae_si_comp.Rda")
load("Data/2_Analysed/Covariates/res_sae_si.Rda")
load("Data/2_Analysed/Covariates/res_sae_int.Rda")
load("Data/2_Analysed/Covariates/res_sae_si_comp.Rda")
load("Data/2_Analysed/Covariates/res_sae_si_quad.Rda")
...
var_lab2 <- tibble(var = c("poblacio_exempta_de_copagament_farmaceutic", "poblacio_amb_rendes_inferiors_a_18_000_euros", "poblacio_amb_rendes_superiors_a_100_000_euros", "poblacio_amb_ocupacions_manuals", "poblacio_amb_nivell_dinstruccio_insuficient", "taxa_de_mortalitat_prematura", "hospitalitzacions_evitables"),
                  lab = c("Population exempted from pharmaceutical co-payment", "Population income < 18k€", "Population income > 100k€", "Population manual employment", "Population with inadequate level of education", "Premature mortality rate", "Avoidable hospitalisations")
                  )
```
...
`r fig_nums("map_pop", "Map of the 2020 population by ABS")`
...
dat_map <- st_as_sf(shapefileT) %>%
  left_join(spob_abs, by = "codi_abs")

pal <- colorNumeric(palette = carto_pal(5, "BrwnYl"), domain = dat_map$N)

map <- leaflet(data = dat_map, options = leafletOptions(zoomControl = FALSE)) %>%
  addProviderTiles(provider = "OpenStreetMap.Mapnik")%>%
  htmlwidgets::onRender("function(el, x) {
        L.control.zoom({ position: 'topleft' }).addTo(this)
    }") %>%
  setView(1.5209,41.5912,zoom=7)%>% 
  addPolygons(group="polygons",weight = 1, color="grey" , smoothFactor = 0.5,highlightOptions = highlightOptions(color = "white", weight = 4,bringToFront = TRUE), fillColor= ~pal(N), fillOpacity = 0.8, layerId=~abs) %>% 
  addLegend(layerId="legend", pal = pal, values = ~N, opacity = 0.5, title = "Population", position = "bottomright") 

map

# saveWidget(map, "temp.html", selfcontained=TRUE)
# webshot::webshot("temp.html", file=r"(C:\Users\psatorra\Documents\TFM\5_Productes\Figures\map_pob_abs.png)", cliprect = c(0, 200, 800, 800))

```

`r fig_nums("map_pop2", "Map of the 2020 population by ABS")`
...
saveWidget(map, "temp.html", selfcontained=TRUE)
webshot::webshot("temp.html", file=r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\Premi Student\Presentation_STUDENT\Figures\figure1.png)", vwidth = 800, vheight = 800)

```

`r fig_nums("map_rs", "Map of the Catalonia ABS and sanitary regions")`
...
pal <- colorFactor(palette = RColorBrewer::brewer.pal(7, "Dark2"), levels = sort(unique(dat_map$NOMRS)))
...
map <- leaflet(data = dat_map, options = leafletOptions(zoomControl = FALSE)) %>%
  ...
  setView(1.5209,41.5912,zoom=8)%>% 
  addPolygons(group="polygons",weight = 1, color="black" , smoothFactor = 0.5,highlightOptions = highlightOptions(color = "white", weight = 4,bringToFront = TRUE), fillColor= ~pal(NOMRS), fillOpacity = 0.8, layerId=~abs) %>% 
  addLegendFactor(layerId="legend", pal = pal, values = ~NOMRS, opacity = 0.5, title = "Health regions", position = "bottomright")
  # addLegendFactor(layerId="legend", pal = pal, values = ~NOMRS, opacity = 0.5, title = "", position = "bottomright", labelStyle = "font-size: 30px;")


saveWidget(map, "temp.html", selfcontained=TRUE)
webshot::webshot("temp.html", file=r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\figure1.png)", vwidth = 800, vheight = 800)
# webshot::webshot("temp.html", file=r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\Premi Student\Presentation_STUDENT\Figures\figure1.png)", vwidth = 800, vheight = 800)
...
```{r echo = FALSE, warning=FALSE, message=FALSE}
p <- ggplot(data = sdat_cat, aes(x = data, y = rate)) +
...
p
p1 <- p
# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\evolution_cas.png)",width = 4000,height = 2000,res = 600)
...
#   theme_classic() +
#   theme(axis.line=element_blank(),axis.title.x=element_blank(), axis.ticks.y = element_blank()) +
#   ylab("Weekly case rate (x100,000 pop.)")
# dev.off()
...
cat <- ggplot(map) + 
       ...
       scale_fill_distiller(name = "Cases (%)", palette = "YlOrRd", direction = 1)
...
# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\map_distribution_cas.png)",width = 3000,height = 2000,res = 600)
# m
# dev.off()
...
# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\evolution_hosp.png)",width = 4000,height = 2000,res = 600)
# ggplot(data = sdat_cat, aes(x = data, y = rate)) +
#   geom_bar(stat = "identity",fill="#DD8888", alpha = 0.5)+
#   scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m") +
#   geom_vline(aes(xintercept = xint), linetype = "dashed") +
#   theme_classic() +
#   theme(axis.line=element_blank(),axis.title.x=element_blank(), axis.ticks.y = element_blank()) +
#   ylab("Weekly hospitalisation rate (x100,000 pop.)")
# dev.off()
```
...
cat <- ggplot(map) + 
       ...
       scale_fill_distiller(name = "Hosp (%)", palette = "YlOrRd", direction = 1)
...
# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\map_distribution_hosp.png)",width = 3000,height = 2000,res = 600)
# m
# dev.off()
```
...
<!-- png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\figure2.png)",width = 8000,height = 4000,res = 600) -->

<!-- ggarrange( -->
<!--   m1 + theme(plot.title = element_text(hjust = 0.5, margin = margin(200, 20, 20, 20))), -->
<!--   p1 + theme(plot.title = element_text(hjust = 0.5, margin = margin(200, 20, 20, 20))), -->
<!--   m2 + theme(plot.title = element_text(hjust = 0.5, margin = margin(200, 20, 20, 20))), -->
<!--   p2 + theme(plot.title = element_text(hjust = 0.5, margin = margin(200, 20, 20, 20))), -->
...
cat <- ggplot(map) + 
  ...
  scale_fill_gradient2(
    name = "RR Cases", midpoint = log(1), low = "blue", mid = "white", high = "red", trans = "log10", breaks = c(0.6, 0.8, 1, 1.25, 1.5)
  ) +
  theme_bw() +
  theme(
    axis.ticks = element_blank(),
...
# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\map_rr_spat_cas.png)",width = 3000,height = 2000,res = 600)
# p1_sp
# dev.off()
...
cat <- ggplot(map) + 
  ...
  scale_fill_gradient2(
    name = "RR Hosp.", midpoint = log(1), low = "blue", mid = "white", high = "red", trans = "log10", breaks = c(0.5, 0.8, 1, 1.25, 2)
  ) +
  theme_bw() +
  theme(
    axis.ticks = element_blank(),
...
p2_sp + ggtitle("Hospitalisations")

# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\map_rr_spat_hosp.png)",width = 3000,height = 2000,res = 600)
# p2_sp
# dev.off()
```
...
p1_temp <- ggplot() +
  ...
  scale_y_continuous(name = "Weekly case rate (x100,000 pop.)", position = "right", trans = "log10", sec.axis = sec_axis(~./150, name = "RR Cases"))

# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\rr_temp_cas.png)",width = 4000,height = 1800,res = 600)
# p1_temp
# dev.off()
...
p2_temp<- ggplot() +
  ...
  scale_y_continuous(name = "Weekly hospitalisation rate (x100,000 pop.)", position = "right", trans = "log10", sec.axis = sec_axis(~./14, name = "RR Hospitalisations"))

# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\rr_temp_hosp.png)",width = 4000,height = 1800,res = 600)
# p2_temp
# dev.off()
...
dat_plot <- tibble(nomrs = rev(sort(unique(rr_sp_temp$NOMRS)))) %>% 
  mutate(
    nomrs = factor(nomrs, levels = sort(unique(nomrs)))
  ) %>% 
  arrange(nomrs) %>% 
  mutate(
    id = row_number(),
...
p1_sptemp <- ggplot(data = dat_plot, aes(x = data, y = rr, group = factor(idarea), color = factor(color), alpha = alpha)) +
        geom_line() +
        ...
        theme_classic() + 
        theme(axis.line=element_blank(),axis.title.x=element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), text = element_text(size = 16), strip.text = element_text(size = 20)) +
        geom_vline(aes(xintercept = xint), linetype = "dashed") +
        ylab("RR Cases")

p1_sptemp

# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\rr_sptemp_cas.png)",width = 10000,height = 6000,res = 800)
# p1_sptemp
# dev.off()
...
p2_sptemp <- ggplot(data = dat_plot, aes(x = data, y = rr, group = factor(idarea), color = factor(color), alpha = alpha)) +
        geom_line() +
        facet_wrap(~nomrs, ncol = 2) +
        geom_hline(yintercept = 1, color = "black", linetype = "dashed") +
        scale_alpha(guide = "none", range = c(0.1, 1)) +
        ...
        theme(axis.line=element_blank(),axis.title.x=element_blank(), axis.ticks.y = element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), text = element_text(size = 16), strip.text = element_text(size = 20)) +
        geom_vline(aes(xintercept = xint), linetype = "dashed") +
        ylab("RR Hospitalisations")

p2_sptemp

# png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\rr_sptemp_hosp.png)",width = 10000,height = 6000,res = 800)
# p2_sptemp
# dev.off()
...
```{r echo = FALSE, warning=FALSE, message=FALSE, eval=FALSE}
png(r"(I:\CTebe\2_Projectes\2023_05TFMPS\4_Productes\2_Figures\figure3.png)", width = 10000,height = 8000,res = 600)
p1 <- ggarrange(
  p1_sp + theme(plot.margin = margin(10, 10, 10, 10)),
  p2_sp + theme(plot.margin = margin(10, 10, 10, 10)),
  p1_temp + theme(plot.margin = margin(10, 10, 10, 10)),
  p2_temp + theme(plot.margin = margin(10, 10, 10, 10)),
  labels = c("a)", "b)", "c)", "d)", "e)", "f)")
)

p2 <- ggarrange(
  p1_sptemp + theme(plot.margin = margin(10, 10, 10, 10)),
  p2_sptemp + theme(plot.margin = margin(10, 10, 10, 10)),
  labels = c("e)", "f)")
) 

ggarrange(p1, p2, widths = c(1, 2), nrow = 2, ncol = 1) +
...
# for(i in 1:nrow(nplot)) {
  # png(str_glue("I:/CTebe/2_Projectes/2023_05TFMPS/4_Productes/2_Figures/linear_covar_cas_{i}.png"), width = 3000,height = 2000,res = 600)
  # print(nplot$plot[[i]])
  # dev.off()
# }
```
...
# for(i in 1:nrow(nplot)) {
  # png(str_glue("I:/CTebe/2_Projectes/2023_05TFMPS/4_Productes/2_Figures/linear_covar_hosp_{i}.png"), width = 3000,height = 2000,res = 600)
  # print(nplot$plot[[i]])
  # dev.off()
# }
...
#   column_spec(2, background = "#4A90E2", color = "white") %>%
#   column_spec(5, background = "#4A90E2", color = "white")
# 
# raw_code <- paste0(as.character(t), collapse = "\n")
# cat(raw_code)
```
