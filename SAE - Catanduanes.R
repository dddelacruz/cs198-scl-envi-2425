...
#---Load data----
load(file = "Data/1_Processed/pob_towns.Rda") # pob_abs
load(file = "Data/1_Processed/dat_cas.Rda") # dat_cas
load(file = "Data/1_Processed/dat_age.Rda") # dat_edat
# load(file = "Data/1_Processed/dat_hosp.Rda")
load(file = "Data/catanduanes_shapefile.Rda")

#Load covariates
load(file = "Data/1_Processed/covariates.Rda") # dat_covar
#We will not load mobility data because it is monthly (we do not have enough variability...)
#We should process the restriction data before incorporating them as covariates
...
Tpob_cas <- pob_town %>% 
  ...
  summarise(NT = sum(count)) %>% 
  rename(edat = AgeGroup)

# add age hospitalization clustering
Tpob_hosp <- pob_town %>% 
  group_by(Year, Gender, AgeGroupHosp) %>% 
  summarise(NT = sum(N)) %>% 
  rename(Age = AgeGroupHosp)

# remove vaccination
# add age vaccination clustering
# Tpob_vac <- pob_town %>%
...
nb <- poly2nb(catanduanes_shapefile)
nb2INLA("map.adj", nb)
g <- inla.read.graph(filename = "map.adj")

#Let's define an index for every poligon
catanduanes_shapefile$idarea <- 1:nrow(catanduanes_shapefile@data)
# this is doable naman
...
res_out <- function(x, inla = FALSE, edat = FALSE){
  ...
  if(edat) {
    ...
    spob_abs <- pob_town %>% 
      filter(AgeGroup %in% c("70 to 74", "75 to 79", "80+")) # %>% 
      mutate(
        AgeGroup = droplevels(AgeGroup),
        AgeGroupHosp = droplevels(AgeGroupHosp)
      )
    
    sdat_hosp <- dat_hosp %>% 
      filter(AgeGroup %in% c("70 to 79", "80+")) %>% 
      mutate(
        AgeGroup = droplevels(AgeGroup)
      )
  ...
  if(!edat) {
    ...
    Tedat_cas <- dat_age %>% 
      ...
      full_join(Tpob_cas %>% filter(Year == year(x)), by = c("Sex", "Age")) %>% 
      mutate(
    ...
    Tedat_cas_total <- dat_age %>% 
      mutate(
        Year = year(data)
      ) %>% 
      left_join(Tpob_cas, by = c("Year", "Sex", "Age")) %>% 
      group_by(Sex, Age) %>% 
      summarise(n = sum(n),
                NT = sum(NT)) %>% 
      mutate(
    ...
    exp_cas <- pob_town %>% 
      ...
      group_by(Sex, Age) %>% 
      summarise(N = sum(N)) %>% 
      left_join(Tedat_cas, by = c("Sex", "Age")) %>% 
      left_join(Tedat_cas_total, by = c("Sex", "Age")) %>% 
      mutate(
        exp = N*ratio,
        Texp = N*Tratio
      ) %>% 
  ...
  Tedat_hosp <- sdat_hosp %>% 
    filter(DateRepConf == x) %>% 
    group_by(Sex, Age) %>% 
    summarise(n = sum(n)) %>% 
    full_join(spob_hosp %>% filter(Year == year(x)), by = c("Sex", "AgeGroup")) %>% 
    mutate(
      n = ifelse(is.na(n), 0, n),
      ratio = n/NT
    ) %>% 
    dplyr::select(Sex, Age, ratio)
